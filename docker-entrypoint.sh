...
# Create health check URL directly
...
urlpatterns = [
    ...
    path('health/', ping, name='health'),
]
EOF

# Insert health check URL at the top of urls.py
echo "Adding health check URL to urls.py"
cat > /app/temp_urls.py << EOF
"""
...
# Append original urls.py content after our new health check URLs
grep -v "^urlpatterns = \[" /app/muadhin/urls.py | tail -n +15 >> /app/temp_urls.py
mv /app/temp_urls.py /app/muadhin/urls.py
...
if [ "$1" = "web" ] || [ -z "$1" ]; then
    ...
    exec gunicorn muadhin.wsgi:application --bind 0.0.0.0:${PORT:-8000} --log-file - --access-logfile - --error-logfile -
elif [ "$1" = "worker" ]; then
